...
def GC(time_unit=0.08, uniform_desired_speed=False, device='cpu'):
    ...
    wall_node = torch.tensor([
        ...
        [35, 35], [35, 29.48], [30, 29.48], [30, 25.62], [35, 25.62], [35, 18.99], [30, 18.99], [30, 14.79],
        [35, 14.79], [35, 7.07], [30, 7.07], [30, 0],
        [30, -5], [0, -5], [0, 0]
    ], device=device)
    ...
    entry = [
        torch.stack((torch.zeros(100, device=device), torch.linspace(5.63 + 1, 16.01 - 1, 100, device=device)), dim=1),
        # Left
        torch.stack((torch.linspace(0 + 1, 5.93 - 1, 100, device=device), 35 * torch.ones(100, device=device)), dim=1),
        # Top(1)
        torch.stack((torch.linspace(21.43 + 1, 30 - 1, 100, device=device), 35 * torch.ones(100, device=device)),
                    dim=1),  # Top(2)
        torch.stack((30 * torch.ones(100, device=device), torch.linspace(29.48 + 1, 35 - 1, 100, device=device)),
                    dim=1),  # Right(1)
        torch.stack((30 * torch.ones(100, device=device), torch.linspace(18.99 + 1, 25.62 - 1, 100, device=device)),
                    dim=1),  # Right(2)
        torch.stack((30 * torch.ones(100, device=device), torch.linspace(7.07 + 1, 14.79 - 1, 100, device=device)),
                    dim=1),  # Right(3)
        torch.stack((torch.linspace(0 + 1, 30 - 1, 100, device=device), torch.zeros(100, device=device)), dim=1),
        # Bottom
    ]
    ...
    def update(frame):
        ...
        exit = torch.argmin(torch.stack(
            [torch.min(torch.norm(frame['destination'].reshape(-1, 1, 2) - e.reshape(1, -1, 2), dim=-1), dim=1).values
             for e in entry], dim=1), dim=1)  # N

        dis2exit = torch.concat(
            [torch.min(torch.norm(frame['position'][(p,), :] - entry[int(exit[p])], dim=-1)).reshape(1) for p in
             range(frame['num_pedestrians'])], dim=0)
        dis2des = torch.norm(frame['position'] - frame['destination'], dim=1)
