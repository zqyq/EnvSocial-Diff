...
class Intest_Aware(nn.Module):
    def __init__(self, img_feat_dim=512, text_feat_dim=768, hidden_dim=256,out_dim=64, n_heads=4,config=None):
        super().__init__()
        self.config = config
    ...
    def forward(self, poi_img_feats, poi_text_feats, poi_boxes,
        ...
        # person_feats = person_feats * mask.unsqueeze(-1)

        if self.config.use_poi_sae:
            ...
            poi_feats = torch.cat([poi_img_feats, poi_text_feats], dim=-1)  # M
            kv_input = poi_feats
        ...
        world = self.pixel_2_world(x, y, H,label) # M 2
        ...
        influence_feat = self.out_proj(attn_output)  # [B, M, output_dim]

        return influence_feat

        # return influence_score  # 每个兴趣点对该行人的影响强度
